 ...
 */
GLvoid
glmDraw(GLMmodel* model, GLuint mode, GLenum renderMode, const std::set<int>& selectedTriangles, GLfloat* selectColor, GLfloat* neutralColor)
{
    static GLuint i;
    ...
    while (group) {
        ...
        for (i = 0; i < group->numtriangles; i++) {
			...
			if (selectedTriangles.find(i) != selectedTriangles.end()) //if select paint blue
				if(selectColor!=NULL) glColor4fv(selectColor);
			else
				if(neutralColor!=NULL) glColor4fv(neutralColor);
...
//				GLfloat* n3v = &model->normals[3 * triangle->nindices[0]];
